"""
DDP implementation tests for TrainSDE.
...
from torchdiff.sde import SchedulerSDE, ForwardSDE, ReverseSDE, TrainSDE
...
def make_components(method="vp"):
    """Create minimal SDE components for testing."""
    scheduler = SchedulerSDE(schedule_type="linear", beta_min=0.1, beta_max=20.0)
    kwargs = {}
    if method == "ve":
        kwargs = {"sigma_min": 0.01, "sigma_max": 50.0}
    fwd = ForwardSDE(scheduler, method=method, pred_type="noise", **kwargs)
    rwd = ReverseSDE(scheduler, method=method, pred_type="noise", **kwargs)
...
class TestSDEDDPSetup:
    ...
    def test_single_gpu_setup(self):
        ...
        trainer = TrainSDE(
            score_net=model, fwd_sde=fwd, rwd_sde=rwd,
...
class TestSDEDDPModelWrapping:
    ...
    def test_wrap_models_not_ddp(self):
        ...
        assert not hasattr(trainer.score_net, 'module')


class TestSDEDDPCheckpoint:
    ...
    def test_checkpoint_save_load_non_ddp(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            trainer2 = TrainSDE(
                score_net=model2, fwd_sde=fwd2, rwd_sde=rwd2,
    ...
    def test_checkpoint_module_prefix_stripping(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            checkpoint = {
                'epoch': 5,
                'model_state_dict_score_net': state_dict,
...
class TestSDEDDPSpawn:
    ...
    def _ddp_worker(rank, world_size, tmpdir, results_dict):
        ...
        try:
            ...
            scheduler = SchedulerSDE(schedule_type="linear", beta_min=0.1, beta_max=20.0)
            fwd = ForwardSDE(scheduler, method="vp", pred_type="noise")
            rwd = ReverseSDE(scheduler, method="vp", pred_type="noise")
            model = TinyDiffNet(channels=1)
            ...
            trainer = TrainSDE.__new__(TrainSDE)
            ...
            trainer.score_net = model.to(trainer.device)
            trainer.fwd_sde = fwd.to(trainer.device)
            trainer.rwd_sde = rwd.to(trainer.device)
            ...
            trainer.warmup_lr_scheduler = TrainSDE.warmup_scheduler(optim, 100)
