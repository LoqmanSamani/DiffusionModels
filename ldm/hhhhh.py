...
class UNet(nn.Module):
    def __init__(
        ...
        # up blocks
        skip_channels = list(reversed(self.down_channels[1:]))
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels, time_embed_dim, num_layers, up_sampling_factor, up_sampling=True, dropout_rate=0.2):
        super().__init__()
        self.num_layers = num_layers
        effective_in_channels = in_channels//2 + skip_channels
        print("in channels: ", in_channels)
        print("skip channels: ", skip_channels)
        print("effective channels: ", effective_in_channels)
        print("out channels: ", out_channels)
        self.conv1 = nn.ModuleList([
            Conv3(         # effective_in_channels
        ...
        self.resnet = nn.ModuleList([
            nn.Conv2d(     # effective_in_channels
                in_channels=effective_in_channels  if i == 0 else out_channels,
                out_channels=out_channels,
                kernel_size=1
            ) for i in range(num_layers)
    ...
    def forward(self, x, skip_connection, embed_time, y=None):
        ...
        output = x
        for i in range(self.num_layers):
            print("it is inside the loop")

            resnet_input = output
            print(f"resnet shape: {resnet_input.shape}")
            output = self.conv1[i](output)
            print(f"After conv 1: {output.shape}")
            output = output + self.time_embedding[i](embed_time)[:, :, None, None]
            print(f"After conv1 + embed-time: {output.shape}")
            output = self.conv2[i](output)
            print(f"After conv2: {output.shape}")
            print(f"resnet shape: {resnet_input.shape}")
            output = output + self.resnet[i](resnet_input)
            print(f"After conv2 + resnet: {output.shape}")
...
class Conv3(nn.Module):
    def __init__(self, in_channels, out_channels, num_groups=8, kernel_size=3, norm=True, activation=True, dropout_rate=0.2):
        ...
        #print(f"Conv3 group_norm num_channels: {self.group_norm.num_channels if norm else 'no norm'}")
...
class Attention(nn.Module):
    def __init__(self, in_channels, num_heads=4, num_groups=8, dropout_rate=0.1):
        super().__init__()
        self.in_channels = in_channels
        ...
        # Add a projection layer to map y to the same dimension as x
        self.y_projection = nn.Linear(512, in_channels)  # Assuming y's embed_dim is 512

    def forward(self, x, y=None):
        batch_size, channels, h, w = x.shape
        assert channels == self.in_channels, f"Expected {self.in_channels} channels, got {channels}"

        # Reshape for attention: (batch_size, channels, h, w) -> (batch_size, channels, h * w)
        x_reshaped = x.view(batch_size, channels, h * w)
        x_reshaped = x_reshaped.permute(0, 2, 1)  # (batch_size, h * w, channels)

        if y is not None:
            # Project y to match x's channel dimension
            y = self.y_projection(y)  # (batch_size, seq_len, 512) -> (batch_size, seq_len, in_channels)
            # Cross-attention: x as query, y as key/value
            out, _ = self.attention(x_reshaped, y, y)
        else:
            # Self-attention
            out, _ = self.attention(x_reshaped, x_reshaped, x_reshaped)

        # Reshape back: (batch_size, h * w, channels) -> (batch_size, channels, h, w)
...
upsample = UpSampling(in_channels=512, out_channels=512, up_sampling_factor=2, conv_block=True, up_sampling=True)
x = torch.randn(32, 512, 16, 16)
print("up-sampling test: ", upsample(x).shape)  # Should be [32, 512, 32, 32]
...
def test_unet_and_encoder():
    ...
    unet = UNet(
        ...
        down_channels=[32, 64, 128, 256],  # Match embed_dim
        mid_channels=[256, 256, 256],             # Match embed_dim
        up_channels=[256, 128, 64, 32],   # Match embed_dim
